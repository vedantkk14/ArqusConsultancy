...
export type QueueTone = 'rose' | 'amber' | 'cyan';

/** One "Needs attention" card: icon, title, count, the first few leads and a "View all" link. */
@Component({
  selector: 'app-queue-section',
  imports: [MatIconModule, QueueRow, RouterLink, Skeleton],
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: {
    class: 'card panel queue-section',
    role: 'region',
    '[attr.data-tone]': 'tone()',
  ...
  template: `
    ...
    @if (skeleton()) {
      @for (i of placeholders; track i) {
        <app-skeleton height="56px" radius="10px" />
      }
  ...
  styles: `
    ...
    /* Unassigned is unusual: when there are any, the card gets a warm wash. */
    :host(.priority) {
      background: color-mix(in srgb, var(--tint-amber) 35%, var(--surface));
    }
  `,
})
export class QueueSection {
  ...
  /** Rows shown in the card; the rest are behind "View all" so the three cards stay even. */
  ...
  /** Extra visual weight for a card that is unusual when non-empty. */
  readonly priority = input(false);
  readonly assign = output<QueueLeadItem>();
  readonly whatsapp = output<QueueLeadItem>();

  protected readonly visible = computed(() => this.items().slice(0, this.maxRows()));
