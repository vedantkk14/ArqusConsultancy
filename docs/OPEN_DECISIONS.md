...
| 12 | **Dashboard for non-admin roles** | Sales Manager now has a real team dashboard (`GET /dashboard/sales-manager`, Dev A). Sales Exec and Project Manager still see a "coming soon" note on /dashboard until their dashboards are specified. |
...
| 15 | **"Lost in period" without a `lost_at` field** | `Lead` has `won_at` but no `lost_at`. The Sales Manager dashboard (`apps/leads/dashboard_manager_views.py`) approximates "lost within the period" with `status=LOST` + `updated_at`, which drifts if a lost lead is edited again later. **Recommended default:** add a real `lost_at` field (set alongside `won_at`/`lost_reason` in `services.change_status()`) the next time this app takes a migration, and switch the dashboard to use it. |
| 16 | **Commission visible to a Sales Manager on their team dashboard?** | **Undecided.** No `SHOW_COMMISSION_TO_EXEC` flag exists yet (no Sales Exec dashboard has been built), so the Sales Manager dashboard shows no commission figures either. Revisit both together. |
